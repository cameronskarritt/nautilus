...
func TestStreamCompletion(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, "", got.stop)
			assertRequest(t, recorder, tt.request, true)
			assertSpan(t, tracer, tt.request, true, "", &tt.wantUsage)
		})
	}
}
...
func assertRequest(tb testing.TB, recorder *requestRecorder, want *llm.Request, stream bool) {
	...
	require.Len(tb, got.Input, 1)

	input, ok := got.Input[0].(map[string]any)
	require.True(tb, ok)
	require.Equal(tb, "message", input["type"])
	require.Equal(tb, string(want.Messages[0].Role), input["role"])
	require.Equal(tb, want.Messages[0].Content, input["content"])
	require.Len(tb, got.Tools, len(want.Tools))
	for i := range want.Tools {
		require.Equal(tb, "function", got.Tools[i].Type)
...
func assertSpan(tb testing.TB, tracer *recordingLLMTracer, req *llm.Request, streaming bool, finish string, usage *llm.Usage) {
	...
	require.Equal(tb, string(enums.ProviderOpenAI), span.call.Provider)
