...
func TestWebhookHistoryScope(t *testing.T) {
	...
	_, err := db.Exec(ctx, `INSERT INTO webhook_events(external_id,organization_id,type,schema_version,idempotency_key,payload,occurred_at) VALUES($1,$2,'webhook.test',1,'test','{"safe":true}',CURRENT_TIMESTAMP)`, eventID, org.ID)
	require.NoError(t, err)
	_, err = db.Exec(ctx, `INSERT INTO webhook_deliveries(external_id,organization_id,webhook_id,event_id,trigger,request_key) SELECT $1,$2,w.id,e.id,'event','test' FROM webhooks w,webhook_events e WHERE w.external_id=$3 AND e.external_id=$4`, deliveryID, org.ID, id, eventID)
	require.NoError(t, err)
