...
4. Deliver availability notifications through Temporal workflows with retries and stable event IDs. The notification implementation must recover from a crash between publishing the document and starting delivery, so a committed publication cannot silently lose its notification. Delivery is at least once, so consumers must tolerate duplicates; repeat intake requests must not create duplicate mail items.
