...
Provider implementation and provisioning, middleware context wiring, and migration of existing TOTP ciphertext follow separately. Organization middleware will resolve the authenticated tenant before attaching its encryptor/decryptor; authentication flows will use the shared user key, including login before a session exists. The existing `ENCRYPTION_KEY` remains in use until its ciphertext has been migrated and its runtime dependency can be removed safely.
